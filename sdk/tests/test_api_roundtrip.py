...
def test_api_wrap_unwrap_roundtrip(tmp_path):
    ...
    # 1) двама получателя
    pub1_b64, sec1 = generate_kem_keypair("ml-kem-768")
    pub2_b64, sec2 = generate_kem_keypair("ml-kem-768")
    ...
    assert bundle["kem_alg"] in ("ml-kem-768", "ML-KEM-768", "Kyber768")
    ...
    dek1 = unwrap_dek(bundle, kid="ops-1", sec_bytes=a_sec.read_bytes(), aad=aad, alg="ml-kem-768")
...
def test_api_wrong_aad_fails(tmp_path):
    ...
    pub_b64, sec = generate_kem_keypair("ml-kem-768")
    ...
    dek = os.urandom(32)
    bundle = wrap_existing_dek_for(dek, recips, aad="good", alg="ml-kem-768")

    # Шифрираме с "good"
    nonce = os.urandom(12)
    c = AES.new(dek, AES.MODE_GCM, nonce=nonce)
    c.update(b"good")
    enc, tag = c.encrypt_and_digest(b"x")

    #Опит за unwrap с лош AAD → дешифрацията трябва да падне при verify (ValueError)
    ...
    # Опит за unwrap с лош AAD → грешката се вдига вътре в unwrap_dek (AES-GCM MAC fail)
    with pytest.raises(ValueError):
        _ = unwrap_dek(bundle, kid="u1", sec_bytes=p_sec.read_bytes(), aad="bad", alg="ml-kem-768")
